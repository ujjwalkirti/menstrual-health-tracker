...
export default function LogScreen() {
	...
	const toggleSymptom = (key: string) => setSymptoms((prev) => (prev.includes(key) ? prev.filter((x) => x !== key) : [...prev, key]));
	...
	return (
		<KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : "height"} keyboardVerticalOffset={64}>
			<ScrollView style={{ backgroundColor: colors.background }} contentContainerStyle={{ padding: 24, paddingBottom: 40 }} keyboardShouldPersistTaps="handled">
				{/* Header */}
				...
				<Section label="Mood" colors={colors}>
					<View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
						{MOODS.map(({ label, emoji }) => (
							<EmojiChip key={label} label={label} emoji={emoji} active={mood === label} onPress={() => setMood(mood === label ? "" : label)} colors={colors} />
						))}
					</View>
				</Section>
					...
					<View style={{ gap: 8 }}>
						{FLOWS.map(({ label, value }, index) => {
							...
							return (
								...
								>
									<Text style={{ width: 52, fontSize: 13, fontWeight: "600", color: active ? colors.brand : colors.textSecondary }}>{label}</Text>
				...
				<Section label="Symptoms" colors={colors}>
					<View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
						{SYMPTOMS.map(({ label, emoji, key }) => (
							<EmojiChip key={key} label={label} emoji={emoji} active={symptoms.includes(key)} onPress={() => toggleSymptom(key)} colors={colors} />
						))}
					</View>
				</Section>
